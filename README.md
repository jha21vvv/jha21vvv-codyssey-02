...
- 용량이 작고 가볍다.(DB와 달리 트랜젝션 로그를 저장하지 않기에 가볍다.)
- 호환성이 뛰나다.

8. Git의 init뜻: 현재 내가 작업하고 있는 폴더를 "이제부터 Git이 관리하는 저장소로 만들겠다!"라고 선언하는 명령어

## 22. 평가용 질문 대답란?
1. 퀴즈와 퀴즈게임의 책임 나누는 기준
- 퀴즈는 퀴즈의 형식과 리스트형식의 자료를 딕셔너리로 바꾸는 영역
- 퀴즈게임은 그외의 모든 영역
2. 입력처리등... 어떤기준으로 분리했나? 
- 기능에 더 적합한 퀴즈게임 클래스에 적합한 항목으로 묶을수있는 범위에 따라 메소드로 구분했음
3. state.json에 읽고 쓰는 기능
- save_data, load_data에서 주로 씀
- 열고 그걸 다른 변수로 저장한후, 필요에 따라 읽고 씀.
4. 커밋 나누는 단위
- 기능 구현후, 수정후 커밋 시킴
5. 클래스를 쓰는 경우와 함수만 쓰는 경우
- 함수만 쓰는 경우는 단일한 개체를 이용하여 뭔가를 하는 경우로 예시를 들면 사과한개를 파는 행위라면
- 클래스는 100개의 사과를 전부 품질, 크기, 당도등을 체크해서, 어떤 종류는 경매에 팔고, 어떤건 소매상에 파는 등의 여러 사과에 개별성과 표준화를 할수 있음
6. state.json 필드 구조, 중첩구조
- 필드구조는 [1,2,3,4]고 중첩 구조는 [[1,2],[3,4]]이다
- 퀴즈별로, 문제, 보기,힌트로 나누어져 있기에 2중 중첩구조가 적당하기에 했음
7. 퀴즈 데이터가 10000개가 넘으면 한계 발생 이유
- 제이슨 파일이 한번에 모든 데이터를 들고오기에 1000개가 넘으면 그게 한번에 읽기는 너무 커서 오류가 생긴다
8. state.json에 손상으로 파싱 실패시, 대응 복구 방안
- 사전에 복구 파일을 마련하거나 클라우드를 통한 복구 파일을 마련해둔다
9. 채점 방식이나 퀴즈 구조 조정
- 정답 채점 방식은 def play_quiz를 수정 해야하고, def add_quiz를 수정할듯 하다,